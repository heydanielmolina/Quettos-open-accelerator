...
The compiler (`sw/quettos/compiler.py`) emits, per model:
...
| `[215:208]` | `sh0` (u8) | first shift (requant `s1`, VRMSNORM `S1`, VSILUMUL 15, VSOFTMAX `FRAC_S`) |
| `[223:216]` | `sh1` (i8) | second shift (requant `sbias`, VRMSNORM `G`, VSILUMUL `sh_h`) |
...
| `0x11` | EMBED | gather `K` bytes of row `TOK` from tiled table `addr_a`; `SREG[sreg_dst] <- meta[addr_m + TOK*8]`; requant consumes `SREG[sreg_src]` (= `sreg_dst`) with unit Sw, `acc = q << 24` |
| `0x20` | VRMSNORM | `vs_src -> vs_dst`; gamma int16 streamed from `addr_a`; `imm32 = eps_c`; `sh0/sh1 = S1/G`; `addr_m` low 24 bits = sqrt(d) sfloat constant; absmax -> `SREG[sreg_dst]` |
| `0x21` | VQUANT | flags: width16/8, use_tracked, group (`vs_aux` = group length -> consecutive `SREG[sreg_dst..]`), scale_mul (`SREG *= sfloat imm32`) |
| `0x22` | VROPE | in place at `vs_src`, `n = heads*64`, table row `addr_a + POS*128`, pairs `(i, i+32)`, shift 14 |
| `0x23` | VSILUMUL | `dst = silu(src) * aux`, shifts `sh0` (15) / `sh1` (to `FRAC_H`), absmax -> SREG |
...
`sim/verilator/csr_defs.hpp`, with a pytest asserting the three agree.
**TBD: generated from `isa.py` once the compiler lands.** The register set is: `CTRL` (START/STEP/RESET), `STATUS`
