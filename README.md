...
- a layered oracle, HF fp32 -> integer golden -> ISA simulator -> RTL: the
  ...
  matches the ISA simulator element by element over the `EMBED`, matrix-vector
  and vector programs, at WB 16 through `make bringup` and at WB 64 and 128
  through `make bringup-sweep`;
- a C++ Verilator harness with a fixed-latency memory model that runs a whole
  ...
  counters, four of the six vector opcodes executing on the vector unit and the
  other two rewritten to `NOP` for the measurement, and prints tokens as they
  leave the RTL;
...
`WB=64, B_MAX=1, VL=4, VSRAM_WORDS=4096`, run under `--traffic`: the real
addresses, strides, meta and partial tiles of the compiled program. A wall clock
is the median of a set of runs, with the extremes of the set and its size;
cycle counts are the core's own counters and are identical run to run.

| Measurement | Value | Command |
|---|---|---|
| Decode token, whole model | 8,088,383 cycles, MAC array active in 95.7% of them, 61.7 read bytes per cycle | `make perf IMAGE=build/images/qwen2.5-0.5b-instruct PERF_ARGS="--traffic --max-new 1 --prompt-ids 1"` |
| Decode token, two layers | 2,643,493 cycles, 98.2% MAC-active, 63.4 read bytes per cycle | `make perf PERF_ARGS="--traffic --max-new 1 --prompt-ids 1"` |
| 36-token prompt + 1 decode token, two layers | 20,023,828 cycles, 94.8% MAC-active, 61.3 read bytes per cycle, 2.8% in the vector unit and 0.10% in sequencing | `make perf` |
| 32-token prompt + 20 decode tokens, whole model | 346,366,037 cycles in 118.9 s (117.3 - 119.5, n=9), 95.1% MAC-active, 61.3 read bytes per cycle | `make perf IMAGE=build/images/qwen2.5-0.5b-instruct PERF_ARGS="--traffic --max-new 20 --prompt-ids <the first 32 ids of prompt.tokens> --eos 999999999"` |
| Verilator simulation rate, `--threads 1` | 3.022 Mcycles/s, median of 15 runs in three batches (2.742 - 3.053) | the two-layer decode-token run above |
| `qcore_top` on xc7 | 101 DSP48E1, 32 RAMB36E1, 15 RAMB18E1, 359 RAM32M, 392 SRL16E, 28,884 LUTs, 19,544 flops; 53,164 cells, 23,212 estimated LCs | `make synth` (Yosys `synth_xilinx -family xc7 -flatten`); the table is the Demo configuration section of `syn/reports/qcore_top.md`, which that command regenerates from its own log |
| RTL against the ISA simulator | 40 of 40 runs match element by element at WB=64 and WB=128 | `make bringup-sweep` |
...
top. Quality is scored one layer up, on the integer golden model against fp32,
...
unit and its three lookup tables. Against the same core without the vector unit
the demo configuration goes from 11,484 to 23,212 estimated LCs and from 69 to
101 DSP48E1; the note in `syn/reports/qcore_top.md` names the commit the earlier
figure was measured at. The cycle rows include the unit's work: 9,677,668 of the
32 + 20 run's cycles, 2.8%, are `STALL_VPU`.
...
`rtl/` holds eighteen files, the package and seventeen modules: the vector
...
from, cycle by cycle. `make bringup` and `make bringup-sweep` run the two
programs of `docs/ISA.md` on `qcore_top` through the C++ harness and on the ISA
simulator and compare every VSRAM element, SREG word, dumped logit, CSR and PERF
counter after every descriptor; `make perf` runs a whole compiled program on
the RTL as a traffic and cycle measurement; and `make synth` runs Yosys
`synth_xilinx` over every
block and over the whole core, regenerating `syn/reports/*.md` from the run and
failing if a report no longer reproduces.

`qcore_vpu_top` executes `VRMSNORM`, `VQUANT`, `VSILUMUL` and `VSUBC`. `VROPE`
and `VSOFTMAX` name passes it does not carry, and a descriptor with either
opcode stops the program at the queue head with `STATUS.ERR`, `FAULT = OPCODE`
and the opcode byte in `FAULT_OP`. Those two passes complete the path from the
traffic measurement to generated text. See
[`docs/ROADMAP.md`](docs/ROADMAP.md) for the order and
[`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) for where the unit sits.
