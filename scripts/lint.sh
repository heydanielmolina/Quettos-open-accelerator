...
#
# Every rtl/*.sv file is linted as its own top, with all rtl/*.sv files on the
# command line so that submodules and the package resolve:
...
# VERILATOR, YOSYS, IVERILOG, RTL_DIR (default rtl), ROM_FILE (absolute path
# used for every ROM_FILE parameter; default the matching rtl/gen/<table>.hex).
set -euo pipefail
...
echo "lint: ${#SV_FILES[@]} file(s) under $RTL_DIR/, ${#TOP_LIST[@]} top(s)"
...
for top in "${TOP_LIST[@]}"; do
  file="$RTL_DIR/$top.sv"
  if [ ! -f "$file" ]; then
    echo "lint: $top: $file not found"; FAIL=1; continue
  fi
  ROM_V=()
  ROM_Y=""
  ROM_I=()
  ...
  if is_package "$file"; then
    ...
    if ! "$VERILATOR" --lint-only -Wall -Wpedantic --top-module "$top" -I"$RTL_DIR" "${SV_FILES[@]}"; then
  ...
  if ! "$VERILATOR" --lint-only -Wall -Wpedantic --top-module "$top" -I"$RTL_DIR" \
       ${ROM_V[@]+"${ROM_V[@]}"} "${SV_FILES[@]}"; then
    echo "lint: $top: verilator FAILED"; FAIL=1
  fi
  YCMD="read_verilog -sv -defer -I$RTL_DIR ${SV_FILES[*]};$ROM_Y hierarchy -check -top $top; proc; opt; check -assert"
  if ! "$YOSYS" -q -l "$WORK/$top.yosys.log" -p "$YCMD" >/dev/null 2>&1; then
    echo "lint: $top: yosys FAILED; log follows"; tail -n 40 "$WORK/$top.yosys.log"; FAIL=1
  fi
  if ! "$IVERILOG" -g2012 -I"$RTL_DIR" -s "$top" -o /dev/null \
       ${ROM_I[@]+"${ROM_I[@]}"} "${SV_FILES[@]}"; then
    echo "lint: $top: iverilog FAILED"; FAIL=1
  fi
done
